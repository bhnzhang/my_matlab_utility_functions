...
D = [10] .* 2*(lambda*z_l)/L;
% D = [20, 5, 1] .* 2*(lambda*z_l)/L;
% D = 100000 * 2*(lambda*z_l)/L;
NA = n0*D/(2*f);
...
for ii = 1:length(D)
    ...
    % Compute the R.S. Propagation to the lens
    z2 = [z];
    [ E_XZ_image, FZ_image, E_fxfz_image ] = RS_prop( x, fx, e_xz_l_plus, z2, lambda0, n0 );
...
e_xz_th = L/(1i*lambda*z_all(end)) .* sinc ( (L/(lambda*z_all(end))) * x);
figure;
plot(x, imag(e_xz_image_all), x, e_x0, '--'); hold on;
plot(x, e_xz_th);
xlabel('x (m)'); ylabel('E field (real)');
title('Field (real) at image and object, for object at f');
xlim( [ -L, L  ] );
legendstr = {};
for ii = 1:length(D)
...
legendstr{end+1} = 'Analytical';
legend(legendstr);
makeFigureNice( [800, 500] );

% plot phase of image?
figure;
plot(x, unwrap(angle(e_xz_image_all) * 180/pi) );
xlabel('x (m)'); ylabel('E field phase (degrees)');
title('Field (phase) at image, for object and image at f');
xlim( [ -L, L  ] );

% plot imaginary part of image?
figure;
plot(x, imag(e_xz_image_all) );
xlabel('x (m)'); ylabel('E field imaginary component');
title('Field (imaginary component) at image, for object and image at f');
xlim( [ -L, L  ] );
